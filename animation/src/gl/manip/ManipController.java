...
public class ManipController implements IDisposable {
     ...
     */
    private boolean parentSpace = true;
    ...
    public ManipController(RenderEnvironment re, Scene s, ControlWindow cw, TimelineViewer t, AnimationEngine en) {
        ...
        // Change default manipulation to rotation (Joy)
        setCurrentManipType(Manipulator.Type.ROTATE);
    }
    @Override
    public void dispose() {
        ...
        switch (type) {
            case Manipulator.Type.TRANSLATE:
                if (currentObject != null) {
                    boolean[] manips = currentObject.sceneObject.translateManip;
                    currentManips[Manipulator.Axis.X] = manips[0] ? Manipulator.TranslateX : null;
                    currentManips[Manipulator.Axis.Y] = manips[1] ? Manipulator.TranslateY : null;
                    currentManips[Manipulator.Axis.Z] = manips[2] ? Manipulator.TranslateZ : null;
                }
                else {
            ...
            case Manipulator.Type.ROTATE:
                if(currentObject != null) {
                    float[] manips = currentObject.sceneObject.rotateManip;
                    currentManips[Manipulator.Axis.X] = manips[0] < manips[1] ? Manipulator.RotateX : null;
                    currentManips[Manipulator.Axis.Y] = manips[2] < manips[3] ? Manipulator.RotateY : null;
                    currentManips[Manipulator.Axis.Z] = manips[4] < manips[5] ? Manipulator.RotateZ : null;
                }
                else {
            ...
            case Manipulator.Type.SCALE:
                if (currentObject != null) {
                    boolean[] manips = currentObject.sceneObject.scaleManip;
                    currentManips[Manipulator.Axis.X] = manips[0] ? Manipulator.ScaleX : null;
                    currentManips[Manipulator.Axis.Y] = manips[1] ? Manipulator.ScaleY : null;
                    currentManips[Manipulator.Axis.Z] = manips[2] ? Manipulator.ScaleZ : null;
                }
                else {
    ...
    public void applyRotation(int axis, Matrix4 mViewProjection, RenderObject object, Vector2 lastMousePos, Vector2 curMousePos) {
        ...
        float[] manips = object.sceneObject.rotateManip;
        float[] rot = object.sceneObject.curRotate;
        Matrix4 M = new Matrix4();
        switch (axis) {
            case Manipulator.Axis.X:
                object.sceneObject.curRotate[0] = Math.min(Math.max(rot[0] + (float)Math.toDegrees(amount), manips[0]), manips[1]);
                if (object.sceneObject.curRotate[0] > manips[0] && object.sceneObject.curRotate[0] < manips[1])
                    M = Matrix4.createRotationX(amount);
                break;
            case Manipulator.Axis.Y:
                object.sceneObject.curRotate[1] = Math.min(Math.max(rot[1] + (float)Math.toDegrees(amount), manips[2]), manips[3]);
                if (object.sceneObject.curRotate[1] > manips[2] && object.sceneObject.curRotate[1] < manips[3])
                    M = Matrix4.createRotationY(amount);
                break;
            case Manipulator.Axis.Z:
                object.sceneObject.curRotate[2] = Math.min(Math.max(rot[2] + (float)Math.toDegrees(amount), manips[4]), manips[5]);
                if (object.sceneObject.curRotate[2] > manips[4] && object.sceneObject.curRotate[2] < manips[5])
    ...
    public void checkPicking(Renderer renderer, RenderCamera camera, int mx, int my) {
        ...
        if (currentObject == null)
            for (int i = 0; i < 3; i++) currentManips[i] = null;
